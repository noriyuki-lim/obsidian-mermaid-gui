...
│   │   ├── EditorShell.tsx    # 非 flowchart エディタ共通のシェル（toolbar / Undo/Redo 履歴 / preview / code pane / 任意の常時表示フルハイト右パネル `sidePanel`）
...
│       ├── EditorModal.ts        # Modal 生成 + toolbar ドラッグ / ダブルクリック最大化 / 最大化ドラッグ復元 + 四隅リサイズハンドル
...
- Modal は toolbar ドラッグで移動、toolbar 空白部分のダブルクリックで最大化 / 復元、四隅のカスタムハンドル（`mge-resize-handle-{nw,ne,sw,se}`）で拡縮できる。最大化 / 復元は短い矩形 transition を入れ、toolbar ドラッグ中と四隅リサイズ中は遅延感を避けるため transition しない。最大化状態から toolbar をドラッグした場合は、掴んだ位置がマウス直下に来るよう復元矩形の `left` / `top` を調整して自由配置へ移行する。初期の中央配置状態では四隅リサイズ時に中心位置を固定して対称に拡縮し、toolbar ドラッグ後は自由配置として掴んだ角だけを動かす。クランプは最小 540×360、最大 98vw×96vh。CSS の `resize: both` は撤去し、grippers は `EditorModal.onOpen` で生成し `onClose` で破棄する。
- **モーダルの開閉アニメーションは opacity フェードに固定する（設計判断）**。`.mge-modal` は `styles.src.css` で opacity のみの `@keyframes mge-modal-open` を `animation ... !important` で当て、コミュニティテーマ（例: "Transparent"）が `.modal` に対して持つ `transform: scale()` 開閉アニメーションを無効化する。React Flow はノード/ハンドル座標を `getBoundingClientRect()` で計測し、その値は祖先要素の transform を巻き込むため、モーダルが `scale()` 中に計測されると flowchart のエッジがハンドルからズレて固定される（座標データ自体は正しく、ドラッグ/Auto-layout が再計測を強制して「直る」ように見えるだけ）。祖先 transform を再計測で後追い補正するのではなく、そもそも scale を持ち込ませないことで開いた最初のフレームから 1:1 で計測されるようにする。`prefers-reduced-motion` ではフェードも無効化。同種の落とし穴は自作 SVG の座標マッピングでも起こりうる。
- `EditorShell` の右ペインに同居する Mermaid ソースは `onSourceEdit` callback の有無で編集可否が決まる。非 flowchart の各エディタは callback を渡し、毎キーストロークで `parse<Kind>` を再実行 → IR に反映 → 失敗時はインライン error バッジで通知（IR は据え置き）。ユーザー入力中は draft を保持し、blur で IR から再生成された canonical 形へスナップする。class / state など `rawItems` 経由で round-trip しているエディタは当該配列を `useState` 管理して再 parse 結果を反映する。Gantt は `previewOverride` で操作可能 SVG を全幅表示し、バー移動・リサイズ・ラベル編集・行並べ替え・タスク追加・依存線 DnD・axisFormat 編集を IR に直接反映する。xychart は `previewOverride` で全幅の操作可能 SVG を表示し、カテゴリ名・系列値の直接編集、棒ドラッグ、縦向き Excel ライクテーブル、TSV ペーストに対応する。block-beta はフォーム中心の `BlockEditor` で items を編集する。kanban は `KanbanInteractivePreview` で全幅 DOM ドラッグボード（カードをカラム間移動、カラム/カード追加・削除・編集）を提供し、`EditorShell` の `sidePanel` に渡す `KanbanOptionsPanel` がプレビュー〜ソース欄の縦幅いっぱいに常時表示される（stacked レイアウトの CSS Grid に `sidepanel` 列を追加し、preview / preview-resizer / body の3行を縦断させている）。`sidePanel` を持つエディタは共有 `children` スロットを使わない前提で、そのぶん `.mge-editor-main-content` を非表示にしてソース欄が行全体を使うようにしている。ソース欄はトグルボタンを維持しつつ `sourceInitiallyOpen` でデフォルト表示。パネルではボード設定（frontmatter の `config.kanban.ticketBaseUrl`。先頭 `---...---` ブロックは kanban に限り検出・温存し、他の図種の判定ロジックには影響しない）と、選択中カードの `ticket`/`assigned`/`priority`（`@{ ... }` メタデータを構造化 read/write）を編集できる。カードには ticket（リンクバッジ）と assigned（氏名そのまま表示、実際の Mermaid レンダリングに合わせてイニシャル化はしない）をボード上にも直接描画し、priority はカード左端ボーダーの色のみで表現する（Mermaid の kanban に任意色指定機能が無いため）。カード以外の場所をクリックすると選択状態を解除する。flowchart は `src/ui/panels/TextPane.tsx` 経由（store の `setText` / `commitText` を blur or debounce で commit）でソースを編集し、`Sort source by canvas` は node / subgraph / edge の出力順を明示的に整列する。ノードはエッジから求めたグラフ上のrank（根からの最長距離）を第一キー、同rankの兄弟同士だけをcross-axis座標（TD/BT系はx、LR/RL系はy）で並べる二段構えで、ピクセル単位の高さのブレに引きずられて意図した左右の並びが崩れないようにしている。並べ替えと同時に Auto-layout（Dagre）も同一コミットで実行し、ソース順とキャンバス座標を1回のUndo単位で揃える。Direction / Subgraph は Palette の Shapes 上部に配置し、Editor edge / Auto-layout は `FlowchartCanvasControls` で canvas 左上に表示する。`Toolbar` は Undo/Redo/Export/Save/Cancel のみ保持する。キャンバスコンテナのリサイズ時は、リサイズ前の画面中心にあった flow 座標をリサイズ後も中心に保つ。ただし Modal 最大化 / 復元の transition 中は viewport 補正を保留し、transition 終了後に1回だけ補正する。Subgraph 選択時の右ペインでは `direction` を `(inherit)` / TD / LR / BT / RL から編集でき、Mermaid ソースの `direction ...` と React Flow キャンバスの局所レイアウトに反映する。ノード選択時の右ペインには connected edges を表示し、edge の選択・削除・source/target 変更を一覧から行える。選択済み edge は canvas 上でも前面化し、edge reconnect は選択済み edge が1本かつヒットした edge と固定端点を共有する場合、その選択済み edge を操作対象にする。Auto-layout は各 subgraph 内を先にレイアウトし、外側の Dagre には表示上の subgraph frame と同じサイズの box node として渡す。Subgraph endpoint の edge はその box node に接続されるため、距離は Dagre の `ranksep` / `nodesep` に集約される。
...
| v0.1.x | 日英 i18n（Obsidian 言語設定に連動）・kanban のカラム並べ替え / frontmatter・カードメタデータ対応・`EditorShell` `sidePanel` 拡張 | 完了 |
