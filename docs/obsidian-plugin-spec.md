...
- Modal は toolbar ドラッグで移動、toolbar 空白部分のダブルクリックで最大化 / 復元、四隅のカスタムハンドル（`mge-resize-handle-{nw,ne,sw,se}`）で拡縮できる。最大化 / 復元は短い矩形 transition を入れ、toolbar ドラッグ中と四隅リサイズ中は遅延感を避けるため transition しない。初期の中央配置状態では四隅リサイズ時に中心位置を固定して対称に拡縮し、toolbar ドラッグ後は自由配置として掴んだ角だけを動かす。クランプは最小 540×360、最大 98vw×96vh。CSS の `resize: both` は撤去し、grippers は `EditorModal.onOpen` で生成し `onClose` で破棄する。
