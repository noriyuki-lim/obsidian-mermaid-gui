...
> **GUI対応:** ✅ 実装済み（DOM ドラッグボード：専用グリップでタスクの列内・列間移動と列の左右並べ替え、ドラッグ中はポインタ追従ゴースト + アニメーション付き並べ替え、カラム / カード追加・削除・編集。右側の詳細設定パネルで ticketBaseUrl / カード単位の ticket・assigned・priority も編集可能。`column <id>[<title>]` キーワード形の列宣言と frontmatter 付きソースにも対応）

ワークフローのカラムとタスクカードを表示するカンバンボード。Obsidian 内蔵 Mermaid が対応しているためプレビューも正常に描画される。Mermaid の kanban には列・カードへの任意色指定機能は無く、カードの見た目に影響する設定は `priority`（Very High / High / Low / Very Low）のみ。
