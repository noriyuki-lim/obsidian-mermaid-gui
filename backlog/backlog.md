...
| 40 | ✅ 完了 | ダークモードでセクション見出し/バッジが背景に溶ける | `.mge-seq-section-title` を `--text-normal` + opacity 0.82 に変更し可読性確保。`.mge-seq-badge` は `--text-accent` を基調に `color-mix` で薄い背景を生成し、`body.theme-dark` で背景濃度と文字色を引き上げ。`.mge-seq-row-label` も `--text-muted` から `--text-normal` + opacity に変更 |
